...
// Set speed (steps)
GLdouble movementSpeed = 50000.0;
GLdouble rotationSpeed = 0.010;
...
void myinit()
{
	...
	cam.SetWorldCoordinates(36000.0, 52000.0);
	// turn collision detection on
	cam.SetCollisionDetectionOn(true);
	// set number of bounding boxes required
	cam.SetNoBoundingBoxes(26);
	// set starting position of user
	//cam.Position(3333, 10450.0, 5478.0, 180.0);  <-------------------- Tav @Mat
	cam.Position(8285, 10617.0, 42000.0, 180.0); //<-------------------- ECL
...
// Keyboard Functions
//--------------------------------------------------------------------------------------
void arrowKeys(int key, int x, int y) //@Aaron
{
	switch (key)
	{
		//Look left
...
void releaseArrow(int key, int x, int y) //@Aaron
{
	switch (key)
	{
		//stop looking left/right
	case GLUT_KEY_LEFT:
	case GLUT_KEY_RIGHT:
...
void keys(unsigned char key, int x, int y) //@Aaron
{
	switch (key)
	{
		//Step left
...
void releaseKeys(unsigned char key, int x, int y) //@Aaron
{
	switch (key)
	{
...
void CreateBoundingBoxes()
{
	...
	// bottom of steps - modified for new area ---------------------------------------- @Mat
	cam.SetAABBMaxX(8, 33808.0);
	cam.SetAABBMinX(8, 31408.0);
	cam.SetAABBMaxZ(8, 4688.0);
	//cam.SetAABBMaxZ(8, 50.0);
	...
	cam.SetAABBMaxZ(12, 46000.0);
	//cam.SetAABBMinZ(12, 0.0);
	cam.SetAABBMinZ(12, 7000.0);
	...
	cam.SetAABBMinX(17, 31540.0);
	cam.SetAABBMaxZ(17, 5000.0);
	cam.SetAABBMinZ(17, -15000.0);

	//new area north
	cam.SetAABBMaxX(18, 31548.0);
	cam.SetAABBMinX(18, 0.0);
	cam.SetAABBMaxZ(18, -14990.0);
	cam.SetAABBMinZ(18, -15000.0);

	//new area west
	cam.SetAABBMaxX(19, 8.0);
	cam.SetAABBMinX(19, 0.0);
	cam.SetAABBMaxZ(19, 5000.0);
	cam.SetAABBMinZ(19, -15000.0);

	//library stairs top cliff
	cam.SetAABBMaxX(20, 4540.0);
	cam.SetAABBMinX(20, 2600.0);
	cam.SetAABBMaxZ(20, 46600.0);
	cam.SetAABBMinZ(20, 45600.0);

	//library stairs RHS
	cam.SetAABBMaxX(21, 4552.0);
	cam.SetAABBMinX(21, 4440.0);
	cam.SetAABBMaxZ(21, 51800.0);
	cam.SetAABBMinZ(21, 46000.0);

	//library stairs bottom
	cam.SetAABBMaxX(22, 6458.0);
	cam.SetAABBMinX(22, 4532.0);
	cam.SetAABBMaxZ(22, 51800.0);
	cam.SetAABBMinZ(22, 51000.0);

	//library stairs LHS
	cam.SetAABBMaxX(23, 6458.0);
	cam.SetAABBMinX(23, 6450.0);
	cam.SetAABBMaxZ(23, 51800.0);
	cam.SetAABBMinZ(23, 45092.0);
}
...
{	
	...
	cam.SetPlains (FLAT_PLAIN, 0.0, 36000.0 , 10450.0, 10450.0, 40000.0, 45000.0);

	//new bit at bottom of hill ------------------------------------------------------- @Mat
	cam.SetPlains(FLAT_PLAIN, 0.0, 36000.0, 9536.0, 9536.0, -15000, 6200.0);
	...
	//steps next to tavern
	step = 10450.0;
	stepLength = 9808.0;
	for (int i = 0; i < 18; i++)
	{
		cam.SetPlains(FLAT_PLAIN, 2600, 4800, step, step, stepLength, stepLength + 42.0);
		step -= 48.0;
		stepLength -= 142.0;
	}

	cam.SetPlains(FLAT_PLAIN, 2600.0, 4800.0, 9536.0, 9536.0, 6200.0, 7180.0);

	
	//steps next to library
	step = 10450.0;
	stepLength = 45600.0;
	for (int i = 0; i < 25; i++)
		if (i == 5 || i == 15)
		{
			cam.SetPlains(FLAT_PLAIN, 4540, 6500, step, step, stepLength, stepLength + 1024.0);
			step -= 48.0;
			stepLength += 1024.0;
		}
		else
		{
			cam.SetPlains(FLAT_PLAIN, 4540, 6500, step, step, stepLength, stepLength + 142.0);
			step -= 48.0;
			stepLength += 142.0;
		}
	...
	/* ------------------------------------------------ Explain --------------------------------------------------
	{
		cam.SetPlains(FLAT_PLAIN, 2600, 6500, step, step, stepLength, stepLength + 42.0);
		step -= 48.0;
		stepLength += 142.0;
	}
	*/
...
void DrawAboveWindowBlock ()
{
	...
	// ABOVE CANTEEN
	tp.CreateDisplayList (YZ, 213, 256.0, 256.0, 2608.0, 10896.0, 6000.0, 0.78, 154.875); //(YZ, 213, 256.0, 256.0, 2608.0, 10896.0, 10000.0, 0.78, 139.25)
...
void DrawPavement ()
{
	...
	tp.CreateDisplayList(XZ, 579, 128.0, 64.0, 2608.0, 9087.0, -1000.0, 17.1, 128); //pavement near pub
...
void DisplayBricks ()
{
	...
	glCallList(396);
	glCallList(397);
	for (int i = 582; i < 584; i++) glCallList(i);
...
void DrawBricks ()
{
	...
	tp.CreateDisplayList (YZ, 122, 128.0, 128.0, 2608.0, 10000.0, 6000.0, 7.0, 274.75);		// long downstairs panel 10000.0 (YZ, 122, 128.0, 128.0, 2608.0, 10000.0, 10000.0, 7.0, 243.5)
	tp.CreateDisplayList (YZ, 123, 128.0, 128.0, 2608.0, 10000.0, 42960.0, 7.0, 21.0);		// end downstairs panel 11088.0 (YZ, 123, 128.0, 128.0, 2608.0, 10000.0, 42960.0, 7.0, 21.0)
	tp.CreateDisplayList (YZ, 124, 128.0, 128.0, 2608.0, 11088.0, 6000.0, 7.0, 309.75);		// long upstairs panel  (YZ, 124, 128.0, 128.0, 2608.0, 11088.0, 10000.0, 7.0, 278.5)
	tp.CreateDisplayList (XY, 136, 128.0, 128.0, 2096.0, 10000.0, 42960.0, 4.0, 7.0);		// corner exit to psc block from canteen - 4.0 (XY, 136, 128.0, 128.0, 2096.0, 10000.0, 42960.0, 4.0, 7.0)
	tp.CreateDisplayList(YZ, 582, 128.0, 128.0, 2608.0, 8980.0, 6000.0, 4.0, 309.75);		// lower section next to new stairs
	tp.CreateDisplayList(YZ, 583, 128.0, 128.0, 2608.0, 9488.0, 6000.0, 4.0, 309.75);		//
...
//new bit
	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_PAVING_1));
	for (int i = 538; i < 557; i++) glCallList(i);

	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_EDGE));
	for (i = 557; i < 576; i++) glCallList(i);

	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_PAVING_1));
	for (i = 573; i < 576; i++) glCallList(i);

	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_EDGE));
	for (i = 576; i < 579; i++) glCallList(i);

	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_PAVING_1));
	for (int i = 584; i < 610; i++) glCallList(i);

	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(STEP_EDGE));
	for (i = 610; i < 636; i++) glCallList(i);			
...
void DrawEntranceSteps ()
{
	...
	step = 10000.0;
	stepLength = 9808.0;
	for (int i = 538; i < 557; i++)
	{
		tp.CreateDisplayList(XZ, i, 1024.0, 512.0, 2600.0, step, stepLength, 2.2, 0.277);
		tp.CreateDisplayList(XY, i + 19, 64.0, 64.0, 2600.0, step - 64.0, stepLength, 35.0, 1.0);
		step -= 48.0;
		stepLength -= 142.0;
	}
	...
	step = 10000.0;
	stepLength = 45600.0;
	for (int i = 584; i < 610; i++)
	{
		if (i == 584 + 5 || i == 584 + 15)
		{
			tp.CreateDisplayList(XY, i + 26, 64.0, 64.0, 4540.0, step - 64, stepLength + 1024, 30.5, 1.0);
			tp.CreateDisplayList(XZ, i, 1024.0, 512.0, 4540.0, step, stepLength, 1.90625, 2);
			step -= 48.0;
			stepLength += 1024.0;
		}
		else 
		{
			tp.CreateDisplayList(XY, i + 26, 64.0, 64.0, 4540.0, step - 64, stepLength + 142, 30.5, 1.0);
			tp.CreateDisplayList(XZ, i, 1024.0, 512.0, 4540.0, step, stepLength, 1.90625, 0.277);
			step -= 48.0;
			stepLength += 142.0;
		}
...
void DisplayGrass ()
{
	...
	glCallList(460);
	glCallList(477);
	for (int i = 461; i < 478; i++) glCallList(i);
...
void DrawGrass ()
{
	...
	//new bit at bottom of hill
	tp.CreateAngledPolygon(477, 64.0, 64.0,  0.0, 31548.0, 31548.0, 0.0,
												9086.0, 9086, 9086, 9086,
											 -15000.0, -15000.0, 6200.0, 6200.0, 1, 1);
...
void DisplayStepBricks ()
{
	...
	for (int j = 0; j < 2; j++)
	{
		...
		glTranslatef(step, 0.0, 0.0);
		glBindTexture(GL_TEXTURE_2D, tp.GetTexture(WALL_BRICK_STEPS));
		for (int i = 508; i < 517; i++) glCallList(i);

		glBindTexture(GL_TEXTURE_2D, tp.GetTexture(WALL_BRICK_STEPS_TOP));
		for (i = 518; i < 523; i++) glCallList(i);

		glPushMatrix();
		glTranslatef(4806.0, 9222.0, 6126.0);
		glRotatef(-18.69, 1.0f, 0.0f, 0.0f);
		glTranslatef(-4806.0, -9222.0, -6126.0);
		glCallList(523);
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(4806.0, 9461.0, 7213.0);
		glRotatef(-20.31, 1.0f, 0.0f, 0.0f);
		glTranslatef(-4806.0, -9462.0, -7213.0);
		glCallList(524);
		glPopMatrix();

		glPushMatrix();
		glTranslatef(4806.0, 9722.0, 8302.0);
		glRotatef(-17.35, 1.0f, 0.0f, 0.0f);
		glTranslatef(-4806.0, -9722.0, -8302.0);
		glCallList(525);
		glPopMatrix();
		...
		glPushMatrix();
		glTranslatef(4806.0, 9939.0, 9332.4);
		glRotatef(-19.83, 1.0f, 0.0f, 0.0f);
		glTranslatef(-4806.0, -9942.0, -9332.4);
		glCallList(526);
		glPopMatrix();
		glPopMatrix();
		step += -64.0;
	...
	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(WALL_BRICK_STEPS_COVER));
	for (int i = 527; i < 532; i++) glCallList(i);
	glPushMatrix();
	glTranslatef(4806.0, 9222.0, 6126.0);
	glRotatef(-18.69, 1.0f, 0.0f, 0.0f);
	glTranslatef(-4806.0, -9222.0, -6126.0);
...
void DrawStepBricks ()
{
	...
	//New section
	tp.CreateDisplayList(YZ, 508.0, 128.0, 128.0, 4860.0, 9914.0, 9872.0, 1.7188, 1.75);
	tp.CreateDisplayList(YZ, 518.0, 32.0, 128.0, 4860.0, 10134.0, 9868.0, 1.0, 1.78);

	tp.CreateDisplayList(YZ, 509.0, 128.0, 128.0, 4860.0, 9530.0, 9006.0, 3.21875, 6.0);
	tp.CreateDisplayList(YZ, 519.0, 32.0, 128.0, 4860.0, 9942.0, 9004.0, 1.0, 2.55);

	tp.CreateDisplayList(YZ, 510.0, 128.0, 128.0, 4860.0, 9350.0, 7918.0, 2.90625, 6.0);
	tp.CreateDisplayList(YZ, 520.0, 32.0, 128.0, 4860.0, 9722.0, 7916.0, 1.0, 3.0);

	tp.CreateDisplayList(YZ, 511.0, 128.0, 128.0, 4860.0, 9158.0, 6830.0, 2.375, 6.0);
	tp.CreateDisplayList(YZ, 521.0, 32.0, 128.0, 4860.0, 9462.0, 6830.0, 1.0, 2.99);

	tp.CreateDisplayList(YZ, 512.0, 128.0, 128.0, 4860.0, 8966.0, 4590.0, 2.0, 15.0);
	tp.CreateDisplayList(YZ, 522.0, 32.0, 128.0, 4860.0, 9222.0, 4590.0, 1.0, 12.0);

	tp.CreateAngledPolygon(513, 128.0, 128.0, 4860.0, 4860.0, 4860.0, 4860.0,
		9094.0, 9094.0, 9462.0, 9094.0,
		5742.0, 6830.0, 6830.0, 5742.0, 5, 1);
	tp.CreateDisplayList(YZ, 523.0, 32.0, 128.0, 4860.0, 9222.0, 6126.0, 1.0, 5.92);

	tp.CreateAngledPolygon(514, 128.0, 128.0, 4860.0, 4860.0, 4860.0, 4860.0,
		9414.0, 9414.0, 9722.0, 9414.0,
		7086.0, 7918.0, 7918.0, 7086.0, 5, 1);
	tp.CreateDisplayList(YZ, 524.0, 32.0, 128.0, 4860.0, 9462.0, 7213.0, 1.0, 5.93);

	tp.CreateAngledPolygon(515, 128.0, 128.0, 4860.0, 4860.0, 4860.0, 4860.0,
		9594.0, 9594.0, 9942.0, 9722.0,
		8302.0, 9006.0, 9006.0, 8302.0, 5, 1);
	tp.CreateDisplayList(YZ, 525.0, 32.0, 128.0, 4860.0, 9722.0, 8302.0, 1.0, 5.82);


	tp.CreateAngledPolygon(516, 128.0, 128.0, 4860.0, 4860.0, 4860.0, 4860.0,
		9914.0, 9914.0, 10134.0, 9914.0,
		9262.0, 9872.0, 9872.0, 9262.0, 5, 1);
	tp.CreateDisplayList(YZ, 526.0, 32.0, 128.0, 4860.0, 9942.0, 9332.4, 1.0, 4.545);

	tp.CreateDisplayList(XZ, 527.0, 64.0, 128.0, 4792.0, 10166.0, 9868.0, 1.0, 1.78);
	tp.CreateDisplayList(XZ, 528.0, 64.0, 128.0, 4792.0, 9974.0, 9004.0, 1.0, 2.55);
	tp.CreateDisplayList(XZ, 529.0, 64.0, 128.0, 4792.0, 9754.0, 7916.0, 1.0, 3.0);
	tp.CreateDisplayList(XZ, 530.0, 64.0, 128.0, 4792.0, 9494.0, 6830.0, 1.0, 2.99);
	tp.CreateDisplayList(XZ, 531.0, 64.0, 128.0, 4792.0, 9254.0, 4590.0, 1.0, 12.0);
	tp.CreateDisplayList(XZ, 532.0, 64.0, 128.0, 4792.0, 9254.0, 6126.0, 1.0, 5.92);
	tp.CreateDisplayList(XZ, 533.0, 64.0, 128.0, 4792.0, 9494.0, 7213.0, 1.0, 5.95);
	tp.CreateDisplayList(XZ, 534.0, 64.0, 128.0, 4792.0, 9754.0, 8302.0, 1.0, 5.82);
	tp.CreateDisplayList(XZ, 535.0, 64.0, 128.0, 4792.0, 9974.0, 9332.4, 1.0, 4.545);
	tp.CreateDisplayList(XY, 536.0, 64.0, 32.0, 4792.0, 10134.0, 10095.84, 1.0, 1.0);
	tp.CreateDisplayList(XY, 537.0, 64.0, 64.0, 4792.0, 9914.0, 10095.84, 1.0, 3.4376);
...
void CreateTextureList()
{
	...
	DrawPavement ();			// 28, 73-94, 240-249, 428, 436, 579
	DrawBricks ();				// 101-110, 112-169, 180-197, 200-201, 390-399, 430-434, 580
	DrawRoof();					// 1-10, 97-100, 170-179, 202-205, 214-222, 250-257, 296-299, 426-427
	DrawEntranceSteps ();		// 258-295, 206-207, 538-578
	DrawExtras ();				// 300-349, 388, 395, 419-421, 429, 435
	DrawLargerTextures ();		// 350-375, 379-387, 389, 414-418, 422-423, 450-453
	DrawLights ();				// 376-378
	DrawBench ();				// 400-413
	DrawStepBricks ();			// 478-507,508-537
